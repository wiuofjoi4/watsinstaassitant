...
export const getRestaurantsOverview = cached(
  async () => {
...
export const getRestaurantDetail = cached(
  async (id: string) => {
    const rows = (await db.execute(sql`
      select
        r.id, r.name, r.created_at as "createdAt",
        ...
        c.custom_system_prompt as "c_customSystemPrompt",
    ...
    const config = row.c_id
      ? {
          ...
          customSystemPrompt: String(row.c_customSystemPrompt ?? ""),
...
export const getRestaurantUsage = cached(
  async (restaurantId: string) => {
    // Run the detail rows and the aggregate in parallel — two round trips to a
    // remote Supabase instead of a sequential 2×RTT waterfall (per restaurant
    // usage tab load).
